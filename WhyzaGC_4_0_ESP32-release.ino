...
const char* release = "WhyzaGC v4.0";  // Version 4 adds MQTT support and a few tidy ups. Not released for ESP8266 at this stage. Use v 3.6 for ESP8266
const char* releasedate = "Released 6/6/2023";
...
unsigned long mqtt_delay = 0;  // mtqq upload interval
TaskHandle_t Task3;
char mqttbuffer[10];
const char* mqttValue = mqttbuffer;
// String sievertsString;

void mqttpub(void* ptParameters) {  // publish CPM data and status to MQTT broker ( and status to syslog and Tlog while we are here )
      ...
      if (WIFIEnable == 1 && (millis() > (uploaddelay + 59999 - timertotal))) {
        if (WiFi.status() != WL_CONNECTED) {
          ...
          if (statusCode == 200) {
            ...
            if (radmon_fail == 1) {
              syslog.log(LOG_INFO, "Connection to radmon.org restablished ;)");
            ...
            if (radmon_fail == false) {
              syslog.log(LOG_INFO, "Alert - Unable to connect to radmon.org");
