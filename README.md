...
Once the sketch is compiled and uploaded, input is via the 3 featherwing buttons. Button A and Button C will cycle up and down through the available display modes. Button B will select histogram display. Buttons B&C together will toggle WiFi disable and enable, which I found easy to select as the buttons are small enough and so close together. See photos below of different display outputs.
