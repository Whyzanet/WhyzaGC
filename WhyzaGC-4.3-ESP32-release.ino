...
const char* release = "WhyzaGC v4.4";  // Version 4 adds MQTT support and a few tidy ups. Not released for ESP8266 at this stage. Use v 3.6 for ESP8266
const char* releasedate = "Released 7/8/2023";

bool WIFIEnable = true;                           // set startup wifi client behaviour
const char* ssid = "whyza_iot";                   // Set wifi SSID
const char* password = "i still hate hospitals";  // Set wifi password
const char* hostname = "WhyzaGC-ESP32";           // Set own hostname
int TZ = +10;                                     // Set local timezone offset from UTC

bool radmonen = true;                 // Set startup radmon.org behaviour. Will enable WIFI if reqd
const char* radmonuser = "whyza";     // Set radmon.org username
const char* radmonpass = "radmon45";  // Set radmon.org password
...
const char* mqtt_user = "whyza";          // Set mqtt username
const char* mqtt_password = "genx";       // Set mqtt password
...
void mqttpub(void* ptParameters) {  // publish CPM data and status to MQTT broker using 2nd CPU
  for (;;) {
    if (MQTTEnable == 1 && (geigerConnected == true)) {
  ...
  if (WiFi.waitForConnectResult() == WL_CONNECTED) {
    ...
    if (NTP_Use_Hostname == 1) {
      ...
      if (WiFi.hostByName(ntphostname, ip)) {
        ...
        Log.print("Failed to resolve NTP server IP address for ");
        syslog.log(LOG_INFO, "Failed to resolve NTP server IP address for ");
        Log.println(ntphostname);
        syslog.log(LOG_INFO, ntphostname);
      }
...
void setup() {
  ...
  if (MQTTEnable == 1 && (MQTT_Use_Hostname == 1)) {
    ...
    if (WiFi.hostByName(mqtthostname, ip)) {
      ...
      Log.print("Failed to resolve IP address for ");
      syslog.log(LOG_INFO, "Failed to resolve IP address for ");
    ...
    if (MQTTEnable == 1 && (MQTT_Use_Hostname == 0)) {
      mqttclient.setServer(mqtt_server, 1883);  // Use when specifying mqtt IP address
      syslog.log(LOG_INFO, "Using configured MQTT broker IP address: ");
      syslog.log(LOG_INFO, mqtt_server.toString());
    }
  }
